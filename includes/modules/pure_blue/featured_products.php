 ...
 * @copyright Copyright 2008-2010 featuredlement
*/
...
if ( (($manufacturers_id > 0 && $_GET['filter_id'] == 0) || $_GET['music_genre_id'] > 0 || $_GET['record_company_id'] > 0) || (!isset($featured_products_category_id) || $featured_products_category_id == '0') ) {
...
if ($num_products_count > 0) {
  ...
  while (!$featured_products->EOF) {
    $products_price = zen_get_products_display_price_pure_blue($featured_products->fields['products_id']);
    ...
    $list_box_contents[$row][$col] = array('params' => 'class="centerBoxContentsFeatured float-left ' . $additional_class . '"',
    'text' => (($featured_products->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) ? '' : '
		...
		<div class="columnar-product-info"><div class="columnar-product-img"><a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . zen_image(DIR_WS_IMAGES . $featured_products->fields['products_image'], $featured_products->fields['products_name'], IMAGE_FEATURED_PRODUCTS_LISTING_WIDTH, IMAGE_FEATURED_PRODUCTS_LISTING_HEIGHT) . '</a></div>') . '<div class="product-box-name"><a href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">' . substr($featured_products->fields['products_name'], 0, 40) . (strlen($featured_products->fields['products_name']) > 40 ? "..." : "") . '</a></div>

			' . '<span class="central-boxes-price">' . COLUMNAR_PRICE . ' ' . '</span>' . $products_price . ' ' . '


            <br /><div class="detail-button"><a class="detail-link" href="' . zen_href_link(zen_get_info_page($featured_products->fields['products_id']), 'cPath=' . $productsInCategory[$featured_products->fields['products_id']] . '&products_id=' . $featured_products->fields['products_id']) . '">'. DETAIL_LINK .'</a></div>
  ...
  if ($featured_products->RecordCount() > 0) {
    if (isset($featured_products_category_id) && $featured_products_category_id !=0) {
      ...
      $title = '<h2 class="centerBoxHeading"><span>' . TABLE_HEADING_FEATURED_PRODUCTS . ($category_title != '' ? ' - ' . $category_title : '') . '</span></h2>';
    } else {
      $title = '<h2 class="centerBoxHeading"><span>' . TABLE_HEADING_FEATURED_PRODUCTS . '</span></h2>';
    }
