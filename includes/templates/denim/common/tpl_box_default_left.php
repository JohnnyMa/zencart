<?php
/**
 * Common Template - tpl_box_default_left.php
 *
 * @package templateSystem
 * @copyright Copyright 2003-2005 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: tpl_box_default_left.php 2975 2006-02-05 19:33:51Z birdbrain $
 
 * 'Denim' Template designed by zen-cart-power.net
 * @copyright Copyright 2011 zen-cart-power.net - 'Denim' template
*/

// choose box images based on box position
  if ($title_link) {
    $title = $title . ' - <a href="' . zen_href_link($title_link) . '">' . BOX_HEADING_LINKS . '</a>';
  }
//
  if ($box_id == "categories") {
    $header_left = 'category-sidebox-header-left';
    $header_right = 'category-sidebox-header-right';
  } else {
    $header_left = '';
    $header_right = '';
  }
//
?>
<!--// bof: <?php echo $box_id; ?> //-->
<div class="leftBoxContainer" id="<?php echo str_replace('_', '-', $box_id ); ?>" style="width: <?php echo $column_width; ?>">
<div class="sidebox-header-left <?php if ($box_id == "categories"){?>category_sidebox_header_left<?php }?>"><h3 class="leftBoxHeading <?php if ($box_id == "categories"){?>category_sidebox_header_right<?php }?>" id="<?php echo str_replace('_', '-', $box_id) . 'Heading'; ?>"><?php echo $title; ?></h3></div>
<?php echo $content; ?>
</div>
<!--// eof: <?php echo $box_id; ?> //-->
